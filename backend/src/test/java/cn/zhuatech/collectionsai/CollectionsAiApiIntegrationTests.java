 ...
 */
@SpringBootTest @AutoConfigureMockMvc class CollectionsAiApiIntegrationTests { @Autowired MockMvc mvc; private String token;
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 @Test void operatorCanScoreInvoice()throws Exception{mvc.perform(post("/api/ai/collections/score").header("Authorization","Bearer "+token).contentType(MediaType.APPLICATION_JSON).content("{\"invoiceNo\":\"INV-88\",\"overdueDays\":75,\"amount\":260000,\"brokenPromiseCount\":2,\"lastContactDays\":18,\"disputeOpen\":true,\"strategicCustomer\":false}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.strategy").value("DISPUTE_RESOLUTION"));}}
